...
const SUPPLIERS_CATEGORY_SLUGS = ["suppliers"];
...
export default function AdminSuppliersPage() {
  ...
  const handleSave = async (data: any) => {
    try {
      const suppliersCat = categories.find(c => c.slug === "suppliers");
      ...
      const posFields = {
        ...
        categoryId: suppliersCat?.id || data.categoryId,
      };
      ...
      if (selectedItem?.id) {
        ...
        showToast("Постачальника оновлено!");
      } else {
        await api.post(endpoints.posSystems.list, cleanData);
        showToast("Постачальника створено!");
      }
      fetchSuppliers();
    } catch (error) {
      throw error;
  ...
  const handleDelete = async (item: Supplier) => {
    ...
    try {
      ...
      showToast("Постачальника видалено!");
      fetchSuppliers();
    } catch (error) {
  ...
  const handleToggleFeatured = async (item: Supplier) => {
    try {
  ...
  const stats = {
    ...
    featured: suppliers.filter((s) => s.isFeatured).length,
    avgRating: suppliers.length
      ? (suppliers.reduce((acc, s) => acc + (s.averageRating || 0), 0) / suppliers.length).toFixed(1)
      : "0.0",
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-zinc-400 mt-1">Постачальники продуктів та товарів для HoReCa</p>
        </div>
      ...
      <div className="grid grid-cols-4 gap-4">
        <Card className="bg-zinc-900/50 border-zinc-800">
          <CardContent className="p-4">
            ...
            <p className="text-sm text-zinc-500">Всього постачальників</p>
          </CardContent>
        </Card>
        <Card className="bg-zinc-900/50 border-zinc-800">
          <CardContent className="p-4">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-48">
                  <DropdownMenuItem asChild>
                    <Link href={`/suppliers/${item.slug}`} target="_blank">
      ...
      <ServiceEditDialog
        ...
        type="suppliers"
        categories={categories.filter(c => SUPPLIERS_CATEGORY_SLUGS.includes(c.slug))}
      />
    </div>
  );
}
